...
2b. **Key repair** — a constrained decoder (Chrome's Prompt API) can corrupt a property key while leaving its prefix intact: `"value "` with a trailing space, `"children enablement"`, `"action交付"`. A key that starts with a known key and then trails off into whitespace or non-ASCII is read as that key; `text-align` or `labelText` are different names and are left alone. Closing the key set in Chrome's native constraint instead was tried and rejected: Nano then emitted only the typed keys and no text or labels at all, so the schema stays open and the repair lives here.
...
8b. **Root resolution** — a declared root that names no component (a schema-constrained model's signature failure: valid JSON, dangling root) yields to the one Page the model wrote; only when there is no such Page is a Page + Stack wrapper synthesized around the orphaned heads.
