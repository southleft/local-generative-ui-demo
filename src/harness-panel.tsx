...
const GENERATION_PIPELINE: Array<{ stage: string; detail: string }> = [
  ...
  { stage: 'Locked focused decoding', detail: 'Chrome is asked for its most-predictable sampling preset (web pages ignore numeric temperature/topK, and the preset needs Chrome’s sampling origin trial; the debug trace shows what each session actually got). LiteRT-LM 0.14 decodes greedily — its WebGPU runtime rejects non-greedy sampling — so the same prompt reproduces the same surface.' },
